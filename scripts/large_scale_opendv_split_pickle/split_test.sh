...
OUTPUT_DIR='/lustre/fsn1/projects/rech/ycy/commun/output_data/opendv_GPT2_LlamaGen/debug'
RUN_NAME='muP_GPT2_split_pickle_debug'
...
#SBATCH --job-name=muP_GPT2_split_pickle_debug_stage_${stage}
#SBATCH -C h100
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
...
srun python $TRAIN_SCRIPT \\
    ...
    optimizer.lr=$LR \\
    model.network.embedding_dim=128 \\
    model.network.nb_heads=1 \\
    data.batch_size=4 \\
    data.num_workers=6 \\
