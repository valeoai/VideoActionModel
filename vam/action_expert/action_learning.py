...
from vam.action_expert.video_action_model import VideoActionModel
...
class ActionLearning(LightningModule):
    ...
    def __init__(
        ...
        """
        Args:
            vam_conf: The configuration for the Video Action Model.
        ...
        if self.flow_sampling == "beta":
            ...
            self.flow_sig_min = self.vam.flow_sig_min
    ...
    def on_before_optimizer_step(self, optimizer: Optional[Optimizer]) -> None:
        ...
        for logger in self.loggers:
            ...
            if self.global_step % self.grad_logging == 0:
                for k, v in self.vam.action_expert.named_parameters():
                    if v.grad is None:
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.hparams.compile and stage == "fit":
            ...
            self.net = torch.compile(self.vam, fullgraph=True)
...
if __name__ == "__main__":
    ...
    vam_conf = config.model.vam_conf
    optimizer_conf = config.model.optimizer_conf

    vam_conf.finetuning_timesteps = 8
    vam_conf.action_config.action_horizon = 6

    model = ActionLearning(vam_conf, optimizer_conf)
    model.to("cuda")
