...
class KVCache(nn.Module):
    """Pre‑allocated K/V cache à la HuggingFace StaticCache.

    Args:
        batch_size:   max batch (B)
        n_heads:      number of attention heads (H)
        head_dim:     head dimension (D)
        max_seq:      maximum sequence length (L_max)
    ...
    def __init__(
        ...
        self.register_buffer("k", torch.zeros(batch_size, n_heads, max_seq, head_dim), persistent=False)
        self.register_buffer("v", torch.zeros_like(self.k), persistent=False)
        self.pos = 0  # next free write index

    def reset(self) -> None:
        """Zero‑fill and rewind position pointer (cheap)."""
        self.k.zero_()
        self.v.zero_()
        self.pos = 0

    def update(self, k_new: torch.Tensor, v_new: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, int]:
        """Write *in‑place* then return full cache + last valid index.

        k_new, v_new: shape (B, H, L_new, D)
        Returns: (k_full, v_full, last_index) where *last_index* is used to
                  build / update the static bool mask.
        """
        L_new = k_new.size(2)
        self.k[..., self.pos : self.pos + L_new, :] = k_new
        self.v[..., self.pos : self.pos + L_new, :] = v_new
        self.pos += L_new
        return self.k, self.v, self.pos - 1


class StaticBoolMask(nn.Module):
    """A single bool tensor of shape (1,1,1,L_max) reused at each decode step."""

    def __init__(self, max_seq: int):
        super().__init__()
        # True → *keep* the position; False → mask it out.
        self.register_buffer("mask", torch.zeros(1, 1, 1, max_seq, dtype=torch.bool), persistent=False)

    def get(self, last_idx: int) -> torch.Tensor:
        """Update in‑place given *index* of the freshly written token and return mask."""
        self.mask.fill_(False)                  # forbid all cols by default
        self.mask[..., : last_idx + 1] = True   # allow 0 .. last_idx inclusive
        return self.mask
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, batch_size:int, dim_model: int, dim_heads: int, block_size: int, attn_scale: float) -> None:
        super().__init__()
        ...
        self.cache = KVCache(batch_size, self.nb_heads, self.dim_heads, block_size)
        self.bool_mask = StaticBoolMask(block_size)

    def forward(self, x: Tensor, prefill: True) -> Tensor:
        ...
        # KV cache update
        k, v, last_idx = self.cache.update(k, v)

        if prefill:
            # During prefill we can rely on is_causal=True (triangle mask) and avoid bool mask
            attn_out = torch.nn.functional.scaled_dot_product_attention(
                q, k, v, attn_mask=None, is_causal=True, scale=attn_scaling
            )
        else:
            # Decode (L==1): supply static bool mask that hides zeros and enforces causality
            mask = self.bool_mask.get(last_idx)
            attn_out = torch.nn.functional.scaled_dot_product_attention(
                q, k, v, attn_mask=mask, is_causal=False, scale=attn_scaling
            )

        y = rearrange(attn_out, "b nb_heads seq dim_head -> b seq (nb_heads dim_head)")  # re-assemble all head outputs side by side
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = CausalSelfAttention(batch_size, dim_model, dim_heads, block_size, attn_scale)
        self.ln_2 = nn.LayerNorm(dim_model, elementwise_affine=False)
        self.mlp = MLP(dim_model, mlp_dim_mult)

    def forward(self, x: Tensor, prefill : bool) -> Tensor:
        x = x + self.attn(self.ln_1(x), prefill)
...
class MupGPT2(nn.Module):
    ...
    def forward_inference(
        ...
        for frame_idx in range(number_of_future_frames):
            ...
            if _count_nb_frames(context) > self.nb_timesteps - 1:
                ...
                self._reset_kv_cache()
                kv_cache_was_reset = True
            for _ in range(self.nb_tokens_per_timestep):
                # Get next token
...
def load_pretrained_gpt(checkpoint_path: str, batch_size:int,  device: torch.device | str = "cuda", tempdir: Optional[str] = None) -> MupGPT2:
