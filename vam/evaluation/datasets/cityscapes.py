...
if __name__ == "__main__":
    ...
    train_dts = CityscapesDataset(
        root="/datasets_local/cityscapes",
        split="train",
    )

    val_dts = CityscapesDataset(
        root="/datasets_local/cityscapes",
        split="val",
    )

    all_used_paths = []
    for dts in [train_dts, val_dts]:
        all_used_paths.extend(dts.image_paths)
        all_used_paths.extend(dts.masks_paths)
    with open("tmp/cityscapes_paths.txt", "w") as f:
        for pth in all_used_paths:
            pth = pth.replace("/datasets_local", "")
            f.write(f"{pth}\n")
    # used to rsync with:
    # rsync --stats -aviur --files-from=tmp/cityscapes_paths.txt /datasets_local jz:xxxx

    print(len(dts))
    print(dts[1]["image"].shape)

    for i in tqdm(range(len(dts))):
        assert os.path.exists(dts.image_paths[i]), f"Image path does not exist: {dts.image_paths}"
        assert os.path.exists(dts.masks_paths[i]), f"Mask path does not exist: {dts.masks_paths}"

        mask = np.asarray(Image.open(dts.masks_paths[i]))
        unique = Counter(mask.flatten().tolist())

        if i % 100 == 0:
            batch = dts[i]
            # save an example image and mask
            img = batch["image"]
            mask = batch["mask"]
            # permute and renormalize
            ...
            plt.axis("off")
            plt.savefig(f"tmp/cityscape_{i}.png")
