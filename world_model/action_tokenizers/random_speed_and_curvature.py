...
class RandSpeedAndCurvatureTokens(nn.Module):
    ...
    def forward(self, ego_to_world_rot, ego_to_world_tran, timestamps, **kwargs):
        ...
        speed_tokens = torch.randint(low=0, high=self.speed_vocab_size, size=(b, t - 1), device=timestamps.device)
        curvature_tokens = torch.randint(low=0, high=self.curvature_vocab_size, size=(b, t - 1), device=timestamps.device)
