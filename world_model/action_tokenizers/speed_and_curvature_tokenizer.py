...
def compute_signed_speed_and_curvature(
    ego_to_world_tran: torch.Tensor, yaws: torch.Tensor
    ...
    Returns:
    - Signed Speeds: A tensor of shape [B, T-1], representing the speeds
      with sign indicating the direction of movement (positive for forward, negative for backward, zero for no movement).
    - Curvatures: A tensor of shape [B, T-1], representing the vehicle's turning behavior
      based on the change in yaw over the distance traveled.

    Assumptions:
    - The time step between each pose is constant (0.5 seconds).
    - The signed speed is calculated as the Euclidean distance
      between positions at consecutive timesteps divided by the time interval,
      with the sign indicating the direction (forward or backward).
    - Curvature is defined as the change in yaw angle per unit of distance traveled.
    - Direction of movement is determined by the dot product of the vehicle's orientation vector
      with the movement vector between consecutive positions.

    Note:
    - A curvature of 0 implies a straight trajectory,
      while non-zero values indicate turning (positive for right, negative for left).
    ...
    curvatures[distances == 0.0] = 0.0
    # = 15cm/0.5s = 30cm/s = 1.08km/h
    curvatures[speeds < 0.15] = 0.0

    # Calculate orientation vectors based on yaws for direction determination
    orientation_vectors = torch.stack(
        (torch.cos(yaws[:, :-1]), torch.sin(yaws[:, :-1]), torch.zeros_like(yaws[:, :-1])), dim=-1
    )
