...
class ActionExpert(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            *(TransformerLayer(attn_dim, head_dim, ffn_expansion, layer_idx, ffn_dim) for layer_idx in range(depth))
        )
    ...
    def configure_optimizers(self, weight_decay, lr, *args, **kwargs):
        ...
        for (wd, lr), params in groups.items():
            ...
            print(
                f"OPTIM GROUP {i} | wd: {wd:.4f} | lr {lr:.4f} | num parameter tensors: {len(params)}, with {num_params:,} parameters"
            )
            i += 1
    ...
    def forward(self, hidden_states, spatial_positions, temporal_positions, inference=False) -> Tensor:
        """
        Args:
