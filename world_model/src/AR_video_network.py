...
def print_model_info(parameters, width, depth, vocab_embed, spatial_pos_embed):
    """
    ...
    print(f'Width: {width} | Depth: {depth} | #Params: {as_M(n_params)} | of which vocab: {as_M(vocab_params)} | of which pos embed {as_M(pos_params)} ')
...
class AutoRegressiveDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = nn.Sequential(
            nn.RMSNorm(width, elementwise_affine=False),
            nn.Linear(width, vocab_size, bias=False)
        )
        ...
        print_model_info(self.parameters(), width, depth, self.vocab_embed, self.spatial_pos_embed)
    ...
    def _init_weights(self, module):
        ...
        elif module is self.out_proj[-1]:
