...
from pos_embedding import apply_rotary_emb 
from kv_cache import KVCache


class TransformerLayer(nn.Module):
    def __init__(self, attn_dim: int, width: int, head_dim: int,  ffn_expansion: int, layer_idx: int, ffn_dim: int = None) -> None:
        super().__init__()
        
        self.layer_idx = layer_idx
        
        if width % head_dim != 0:
            raise ValueError(f"width {width} is not divisible by head_size {head_dim}")
