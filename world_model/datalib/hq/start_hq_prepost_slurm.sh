...
job_id=$(sbatch --parsable $SCRIPT_DIR/start_hq_server_prepost.slurm)
...
echo "Starting $NUM_WORKERS HQ workers"

# Start workers
job_ids=()
for i in $(seq 1 "$NUM_WORKERS"); do
    echo "Starting worker $i"
    job_id=$(sbatch --parsable $SCRIPT_DIR/start_hq_worker_prepost.slurm)
    if [ $? -ne 0 ]; then
