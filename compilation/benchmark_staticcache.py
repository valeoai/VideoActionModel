...
from vam.video_pretraining.inference_mup_gpt2 import load_pretrained_gpt  
...
gpt  = load_pretrained_gpt(ckpt, batch_size=BS, device=device).to(dtype)

# 2.  Compile ONLY forward_inference  -----------------------
gpt.forward = torch.compile(gpt.forward,
                          fullgraph=True,
                          dynamic=True,
                          mode="max-autotune")

# 3.  Dummy burn-in tokens ---------------------------------
HEIGHT, WIDTH = 18, 32
...
# -----------------------------------------------------------
# 4.  Warm-up (fills the kernel cache, avoids first-run noise)
